...
await pause(600);
console.log("spread start", await evalJs("({visible:getComputedStyle(document.querySelector('[data-project-story]')).display,first:document.querySelector('[data-project-story] [aria-hidden] > div')?.style.transform,scrollWidth:document.documentElement.scrollWidth})"));
await screenshot("portfolio-spread-start-desktop");
await evalJs("window.scrollBy({top:600,behavior:'instant'})");
await pause(500);
console.log("spread progress", await evalJs("({progress:document.querySelector('[data-project-story]')?.style.getPropertyValue('--spread-progress'),first:document.querySelector('[data-project-story] [aria-hidden] > div')?.style.transform})"));
...
await pause(200);
console.log("experience tab", await evalJs("({selected:document.querySelector('.experience-timeline button[aria-selected=true]')?.innerText.slice(0,75),detailsOpen:document.querySelector('.experience-detail-panels article[data-active=true] details')?.open})"));
...
await pause(300);
console.log("reduced motion", await evalJs("({enabled:matchMedia('(prefers-reduced-motion: reduce)').matches,spread:getComputedStyle(document.querySelector('[data-project-story]')).display,gallery:getComputedStyle(document.querySelector('#project-gallery')).display,sticky:getComputedStyle(document.querySelector('.experience-explorer').firstElementChild).position})"));
await send("Emulation.setEmulatedMedia", { features: [{ name: "prefers-reduced-motion", value: "no-preference" }] });
await pause(200);
