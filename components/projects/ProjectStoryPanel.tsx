...
export function ProjectStoryPanel({ project, index, active }: { project: Project; index: number; active: boolean }) {
  ...
  return (
    <article className={styles.quest} data-active={active} data-quest={index} aria-labelledby={`quest-${project.slug}`}>
      {cover ? <Image className={styles.preview} src={cover.src} alt={cover.alt} width={720} height={450} sizes="(max-width: 700px) 100vw, 50vw" /> : null}
